...
void Pais::setValores(string nom, string reg, float pob, float a, float denP, float liCo, float mig, float moIn, float prodIB, float alf, float telefX100, float tieAr, float cult, float otro, float clim, float nat, float mort, float ag, float ind, float serv){
    nombre=nom;
    region=reg;
    poblacion=pob;
    area=a;
